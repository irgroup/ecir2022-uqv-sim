...
for sim in tqdm(['uqv_1', 'uqv_2', 'uqv_3', 'uqv_4', 'uqv_5', 'uqv_6', 'uqv_7', 'uqv_8',
    ...
    for topic in topics.keys():
        ...
        for query in queries:
            ...
            scores['ndcg'].append(rpd_eval.rel_eval.evaluate(parse_results(results, topic=str(topic))).get(str(topic)).get('ndcg'))
            scores['P_10'].append(rpd_eval.rel_eval.evaluate(parse_results(results, topic=str(topic))).get(str(topic)).get('P_10'))
            scores['map'].append(rpd_eval.rel_eval.evaluate(parse_results(results, topic=str(topic))).get(str(topic)).get('map'))
            number_of_queries += 1
...
pd.DataFrame.from_dict(data).to_csv('data/experimental_results/arp.csv')
