...
for sim in tqdm(['uqv_1', 'uqv_2', 'uqv_3', 'uqv_4', 'uqv_5', 'uqv_6', 'uqv_7', 'uqv_8',
    ...
    for topic in topics.keys():
        ...
        for query in queries:
            ...
            ndcg_tmp = rpd_eval.rel_eval.evaluate(parse_results(results, topic=str(topic))).get(str(topic)).get('ndcg')
            P_10_tmp = rpd_eval.rel_eval.evaluate(parse_results(results, topic=str(topic))).get(str(topic)).get('P_10')
            map_tmp = rpd_eval.rel_eval.evaluate(parse_results(results, topic=str(topic))).get(str(topic)).get('map')

            if ndcg_tmp > ndcg_max:
                ndcg_max = ndcg_tmp
            if P_10_tmp > P_10_max:
                P_10_max = P_10_tmp
            if map_tmp > map_max:
                map_max = map_tmp

        scores['ndcg'].append(ndcg_max)
        scores['P_10'].append(P_10_max)
        scores['map'].append(map_max)
...
pd.DataFrame.from_dict(data).to_csv('data/experimental_results/arp_max.csv')
